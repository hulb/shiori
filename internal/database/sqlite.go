...
func OpenSQLiteDatabase(ctx context.Context, databasePath string) (*SQLiteDatabase, error) {
	// Open database and start transaction
	db := sqlx.MustConnect("sqlite3", databasePath)
	sqliteDB := &SQLiteDatabase{dbbase: dbbase{*db}}

	if err := sqliteDB.withTx(ctx, func(tx *sqlx.Tx) error {
		// Create tables
		...
		_, err = tx.ExecContext(ctx, `CREATE VIRTUAL TABLE IF NOT EXISTS bookmark_content USING fts4(title, content, html)`)
		if err != nil {
			return errors.WithStack(err)
		}

		// Alter table if needed
		_, err = tx.ExecContext(ctx, `ALTER TABLE account ADD COLUMN owner INTEGER NOT NULL DEFAULT 0`)
		if err != nil {
			return errors.WithStack(err)
		}
		_, err = tx.ExecContext(ctx, `ALTER TABLE bookmark ADD COLUMN public INTEGER NOT NULL DEFAULT 0`)
		if err != nil {
			return errors.WithStack(err)
		}
		_, err = tx.ExecContext(ctx, `ALTER TABLE bookmark ADD COLUMN processed INTEGER NOT NULL DEFAULT 0`)
		if err != nil {
			return errors.WithStack(err)
		}
		_, err = tx.ExecContext(ctx, `ALTER TABLE bookmark ADD COLUMN create_archive INTEGER NOT NULL DEFAULT 0`)
		if err != nil {
			return errors.WithStack(err)
		}
...
func (db *SQLiteDatabase) SaveBookmarks(ctx context.Context, bookmarks ...model.Bookmark) ([]model.Bookmark, error) {
	...
	if err := db.withTx(ctx, func(tx *sqlx.Tx) error {
		...
		stmtInsertBookContent, err := tx.PreparexContext(ctx, `INSERT OR IGNORE INTO bookmark_content
		...
		for _, book := range bookmarks {
			...
			_, err = stmtUpdateBookContent.ExecContext(ctx, book.Title, book.Content, book.HTML, book.ID)
			if err != nil {
				return errors.WithStack(err)
			}
...
func (db *SQLiteDatabase) SaveAccount(ctx context.Context, account model.Account) error {
	if err := db.withTx(ctx, func(tx *sqlx.Tx) error {
		...
		_, err = tx.ExecContext(ctx, `INSERT INTO account
